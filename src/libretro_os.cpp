...
Common::MutexInternal *createLibretroMutexInternal() { return new LibretroMutexInternal(); }

struct RetroPalette {
  unsigned char _colors[256 * 3];

  RetroPalette() { memset(_colors, 0, sizeof(_colors)); }

  void set(const byte *colors, uint start, uint num) { memcpy(_colors + start * 3, colors, num * 3); }

  void get(byte *colors, uint start, uint num) const { memcpy(colors, _colors + start * 3, num * 3); }

  unsigned char *getColor(uint aIndex) const { return (unsigned char *)&_colors[aIndex * 3]; }
};
...
public:
  ...
  virtual bool hasFeature(Feature f) { return (f == OSystem::kFeatureCursorPalette); }
  ...
  virtual bool getFeatureState(Feature f) { return (f == kFeatureCursorPalette) ? _mousePaletteEnabled : false; }
  ...
  virtual bool setGraphicsMode(int mode) { return true; }

  virtual int getGraphicsMode() const { return 0; }

  virtual void initSize(uint width, uint height, const Graphics::PixelFormat *format) { _gameScreen.create(width, height, format ? *format : Graphics::PixelFormat::createFormatCLUT8()); }

  virtual int16 getHeight() { return _gameScreen.h; }

  virtual int16 getWidth() { return _gameScreen.w; }

  virtual Graphics::PixelFormat getScreenFormat() const { return _gameScreen.format; }
...
protected:
  // PaletteManager API
  virtual void setPalette(const byte *colors, uint start, uint num) { _gamePalette.set(colors, start, num); }

  virtual void grabPalette(byte *colors, uint start, uint num) const { _gamePalette.get(colors, start, num); }
...
public:
  ...
  virtual Graphics::Surface *lockScreen() { return &_gameScreen; }
  ...
  virtual int16 getOverlayHeight() { return _overlay.h; }

  virtual int16 getOverlayWidth() { return _overlay.w; }

  virtual Graphics::PixelFormat getOverlayFormat() const { return _overlay.format; }
  ...
  virtual Common::MutexInternal *createMutex(void) { return createLibretroMutexInternal(); }
...
#define PI 3.141592653589793238
  ...
  void processMouse(retro_input_state_t aCallback, int device, float gampad_cursor_speed, float gamepad_acceleration_time, bool analog_response_is_quadratic, int analog_deadzone, float mouse_speed) {
    ...
    static const unsigned gampad_numpad_map[8][2] = {
        {(unsigned)Common::KEYCODE_KP1, 49}, {(unsigned)Common::KEYCODE_KP2, 50}, {(unsigned)Common::KEYCODE_KP3, 51}, {(unsigned)Common::KEYCODE_KP6, 54}, {(unsigned)Common::KEYCODE_KP9, 57}, {(unsigned)Common::KEYCODE_KP8, 56}, {(unsigned)Common::KEYCODE_KP7, 55}, {(unsigned)Common::KEYCODE_KP4, 52},
    };
...
const Graphics::Surface &getScreen() { return dynamic_cast<OSystem_RETRO *>(g_system)->getScreen(); }
